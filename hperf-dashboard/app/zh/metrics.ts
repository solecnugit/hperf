...
const fnTable = {
    average: (data: number[]) => data.reduce((acc, curr) => acc + curr, 0) / data.length,
    max: (data: number[]) => Math.max(...data),
    min: (data: number[]) => Math.min(...data),
    sum: (data: number[]) => data.reduce((acc, curr) => acc + curr, 0),
};

export function compute(data: TimeSeriesData[], func: "average" | "max" | "min" | "sum"): TimeSeriesData {
...
export function computeOne(data: TimeSeriesData[], func: "average" | "max" | "min" | "sum", field: NumericFields): number {
