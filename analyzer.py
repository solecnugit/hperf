...
class Analyzer:
        ...
        for item in self.event_groups.events:
            if "type" in item:
                if item["type"] == "SYSTEM":
                    ...
                    # ... 
                    # -------------- faulty --------------
                    t_times = len(perf_raw_data.groupby(["timestamp"]).groups.keys())
                    perf_raw_data.loc[perf_raw_data.metric == item["perf_name"], ["unit"]] = ["SOCKET0", "SOCKET1"] * t_times
    ...
    def get_timeseries_plot(self):
        ...
        sar_metrics = [r"%user", r"%nice", r"%system", r"%iowait", r"%steal", r"%idle"]
